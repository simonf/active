...
(function() {
  var cleanAndSortByAction, currentDataset, drawSelectedSeries, fetchAndPlotSelectedCategory, fetchDataForCategory, fetchDistinctCategories, findMinMax, getMood, initCap, insertMissingDays, makeFlotDataObject, makeQuantityNumeric, makeTime, normalise, plotData, sortFunction, stripTrailingZeroes;
  ...
  getMood = function(callback) {
    jQuery.get("/mood", function(dat) {
      callback(dat);
    });
  };
  ...
  findMinMax = function(dataArray, subArrayIndex) {
    ...
    for (_i = 0, _len = dataArray.length; _i < _len; _i++) {
      point = dataArray[_i];
      if (point[subArrayIndex] < min) {
        min = point[subArrayIndex];
      }
      if (point[subArrayIndex] > max) {
        max = point[subArrayIndex];
      }
    }
    return [min, max];
  };
  ...
  plotData = function(dataObject) {
    var fixedUpData, ignore, label, normalisedData, series, _ref, _ref1;
    $('#series').empty();
  ...
  drawSelectedSeries = function() {
    ...
    for (label in currentDataset) {
      ...
      if (checkedArray.indexOf(label) > -1) {
        toDraw.push(series);
      }
    }
    options = {
